...
public class MyRenderer implements GLSurfaceView.Renderer {
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        // Положение камеры (0, 0, 3)
        float eyeX = 0.0f;
        float eyeY = 0.0f;
        float eyeZ = 3.0f;
        // Направление, смотрящее на сферу (0, 0, 0)
        float centerX = 0.0f;
        float centerY = 0.0f;
        float centerZ = 0.0f;
        // Направление "вверха" для камеры
        // Вверх для нас - y
        float upX = 0.0f;
        float upY = 1.0f;
        float upZ = 0.0f;
...
//        Matrix.rotateM(mMVPMatrix, 0, mRotationAngle, 0, 1, 0);
        ...
        float[] ambientColor = {2.2f, 2.2f, 2.2f, 1.0f}; //изначальная освещённость объекта
        float[] diffuseColor = {0.0f, 0.0f, 1.0f, 1.0f}; //рассеивание света
        float[] specularColor = {1.0f, 1.0f, 1.0f, 1.0f};//блики
        float[] objectColor = {1.0f, 0.5f, 0.0f, 1.0f};  //цвет объекта
        float shininess = 100.0f; //блеск
